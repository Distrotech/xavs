...
int xavs_visualize_init( xavs_t *h )
{
...
/* }}} */
/* {{{ [fold] void xavs_visualize_mb( xavs_t *h ) */
void xavs_visualize_mb( xavs_t *h )
{
    ...
    for (y=0; y<4; y++) for (x=0; x<4; x++)
        v->intra4x4_pred_mode[y][x] = h->mb.cache.intra4x4_pred_mode[XAVS_SCAN8_0+y*8+x];
    for (l=0; l<2; l++) for (y=0; y<4; y++) for (x=0; x<4; x++) {
        for (i=0; i<2; i++) {
            v->mv[l][y][x][i] = h->mb.cache.mv[l][XAVS_SCAN8_0+y*8+x][i];
        }
        v->ref[l][y][x] = h->mb.cache.ref[l][XAVS_SCAN8_0+y*8+x];
    }
    v->i_intra16x16_pred_mode = h->mb.i_intra16x16_pred_mode;
}
/* }}} */
/* {{{ [fold] void xavs_visualize_close( xavs_t *h ) */
void xavs_visualize_close( xavs_t *h )
{
    xavs_free(h->visualize);
}
/* }}} */
/* {{{ [fold] void xavs_visualize_show( xavs_t *h ) */
/* Display visualization (block types, MVs) of the encoded frame */
/* FIXME: B-type MBs not handled yet properly */
void xavs_visualize_show( xavs_t *h )
{
    int mb_xy;
        ...
        if (v->i_type==P_L0 || v->i_type==P_8x8 || v->i_type==P_SKIP) {
            ...
            if (v->i_partition==D_8x8) {
                for (i=0; i<2; i++) for (j=0; j<2; j++) {
                    ...
                    l = xavs_mb_partition_listX_table[0][sp] ? 0 : 1; /* FIXME: not tested if this works */
                    if (IS_SUB8x8(sp)) {
