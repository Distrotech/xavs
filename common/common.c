...
void    xavs_param_default( xavs_param_t *param )
{
    ...
    param->i_threads = XAVS_THREADS_AUTO;
...
static int xavs_atobool( const char *str, int *b_error )
{
    if( !strcmp(str, "1") ||
        !strcmp(str, "true") ||
        !strcmp(str, "yes") )
...
static int xavs_atoi( const char *str, int *b_error )
{
    char *end;
...
static double xavs_atof( const char *str, int *b_error )
{
    char *end;
...
int xavs_param_parse( xavs_param_t *p, const char *name, const char *value )
{
    char *name_buf = NULL;
    int b_error = 0;
    int name_was_bool;
...
#define OPT2(STR0, STR1) else if( !strcmp( name, STR0 ) || !strcmp( name, STR1 ) )
    ...
    OPT("asm")
    {
        p->cpu = isdigit(value[0]) ? atoi(value) :
                 !strcmp(value, "auto") || atobool(value) ? xavs_cpu_detect() : 0;
        if( b_error )
        {
            ...
            {
                for( i=0; xavs_cpu_names[i].flags && strcasecmp(tok, xavs_cpu_names[i].name); i++ );
                p->cpu |= xavs_cpu_names[i].flags;
                if( !xavs_cpu_names[i].flags )
                    b_error = 1;
            }
            free( buf );
        }
    ...
    OPT("overscan")
        b_error |= parse_enum( value, xavs_overscan_names, &p->vui.i_overscan );
    OPT("videoformat")
        b_error |= parse_enum( value, xavs_vidformat_names, &p->vui.i_vidformat );
    OPT("fullrange")
        b_error |= parse_enum( value, xavs_fullrange_names, &p->vui.b_fullrange );
    OPT("colorprim")
        b_error |= parse_enum( value, xavs_colorprim_names, &p->vui.i_colorprim );
    OPT("transfer")
        b_error |= parse_enum( value, xavs_transfer_names, &p->vui.i_transfer );
    OPT("colormatrix")
        b_error |= parse_enum( value, xavs_colmatrix_names, &p->vui.i_colmatrix );
    OPT("chromaloc")
    {
    ...
    OPT("cqm")
    {
        if( strstr( value, "flat" ) )
            p->i_cqm_preset = XAVS_CQM_FLAT;
        else if( strstr( value, "jvt" ) )
            p->i_cqm_preset = XAVS_CQM_JVT;
        else
    ...
    OPT("cqm4")
    {
        p->i_cqm_preset = XAVS_CQM_CUSTOM;
    ...
    OPT2("analyse", "partitions")
    {
        ...
        if( strstr( value, "i4x4" ) )  p->analyse.inter |= XAVS_ANALYSE_I4x4;
        if( strstr( value, "i8x8" ) )  p->analyse.inter |= XAVS_ANALYSE_I8x8;
        if( strstr( value, "p8x8" ) )  p->analyse.inter |= XAVS_ANALYSE_PSUB16x16;
        if( strstr( value, "p4x4" ) )  p->analyse.inter |= XAVS_ANALYSE_PSUB8x8;
        if( strstr( value, "b8x8" ) )  p->analyse.inter |= XAVS_ANALYSE_BSUB16x16;
    }
    OPT("8x8dct")
    ...
    OPT2("direct", "direct-pred")
        b_error |= parse_enum( value, xavs_direct_pred_names, &p->analyse.i_direct_mv_pred );
    OPT("chroma-qp-offset")
        p->analyse.i_chroma_qp_offset = atoi(value);
    OPT("me")
        b_error |= parse_enum( value, xavs_motion_est_names, &p->analyse.i_me_method );
...
void xavs_log( xavs_t *h, int i_level, const char *psz_fmt, ... )
{
    if( !h || i_level <= h->param.i_log_level )
    {
        ...
        if( !h )
            xavs_log_default( NULL, i_level, psz_fmt, arg );
        else
...
static void xavs_log_default( void *p_unused, int i_level, const char *psz_fmt, va_list arg )
{
    char *psz_prefix;
    switch( i_level )
    {
...
char *xavs_param2string( xavs_param_t *p, int b_res )
{
    ...
    s += sprintf( s, " rc=%s mbtree=%d", p->rc.i_rc_method == XAVS_RC_ABR ?
                               ( p->rc.b_stat_read ? "2pass" : p->rc.i_vbv_buffer_size ? "cbr" : "abr" )
                               : p->rc.i_rc_method == XAVS_RC_CRF ? "crf" : "cqp", p->rc.b_mb_tree );
    if( p->rc.i_rc_method == XAVS_RC_ABR || p->rc.i_rc_method == XAVS_RC_CRF )
    {
        if( p->rc.i_rc_method == XAVS_RC_CRF )
    ...
    else if( p->rc.i_rc_method == XAVS_RC_CQP )
        s += sprintf( s, " qp=%d", p->rc.i_qp_constant );
    if( !(p->rc.i_rc_method == XAVS_RC_CQP && p->rc.i_qp_constant == 0) )
    {
