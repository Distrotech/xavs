...
extern const vlc_t xavs_coeff0_token[5];
extern const vlc_t xavs_coeff_token[5][16*4];
extern const vlc_t xavs_total_zeros[15][16];
extern const vlc_t xavs_total_zeros_dc[3][4];
extern const vlc_t xavs_run_before[7][16];
...
static const uint8_t xavs_ue_size_tab[256] =
{
...
{
    bs_write( s, xavs_ue_size_tab[val+1], val+1 );
}
...
static inline int bs_size_ue_big( unsigned int val )
{
    if( val < 255 )
        ...
        return xavs_ue_size_tab[(val+1)>>8] + 16;
}
...
static inline int bs_size_se( int val )
{
    ...
    if( tmp < 0 ) tmp = val*2;
    if( tmp < 256 )
        return xavs_ue_size_tab[tmp];
    else
        return xavs_ue_size_tab[tmp>>8]+16;
}
