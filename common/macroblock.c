...
void xavs_mb_load_mv_direct8x8( xavs_t *h, int idx )
{
    ...
    for( l = 0; l < 2; l++ )
    {
        *(uint64_t*)h->mb.cache.mv[l][xavs_scan8[idx*4]] =
        *(uint64_t*)h->mb.cache.direct_mv[l][xavs_scan8[idx*4]];
        *(uint64_t*)h->mb.cache.mv[l][xavs_scan8[idx*4]+8] =
        *(uint64_t*)h->mb.cache.direct_mv[l][xavs_scan8[idx*4]+8];
    }
}
...
static inline void xavs_mb_mc_01xywh( xavs_t *h, int x, int y, int width, int height )
{
	...
	if(x==0 && y==0)
		...
		i=3;
	if(h->mb.i_type==B_SKIP||h->mb.i_type==B_DIRECT
...
void xavs_macroblock_cache_load( xavs_t *h, int i_mb_x, int i_mb_y )
{
    ...
    h->mb.i_neighbour8[0] = (h->mb.i_neighbour & (MB_TOP|MB_LEFT|MB_TOPLEFT))
							...
							| ((h->mb.i_neighbour & MB_LEFT) ? MB_DOWNLEFT : 0); 
    h->mb.i_neighbour4[4] =
...
void xavs_macroblock_cache_save( xavs_t *h )
{
	...
	if( i_mb_type == I_8x8 )
        ...
        h->mb.intra4x4_pred_mode[i_mb_xy][6] = -1;//I_PRED_8x8_DC;
		}
