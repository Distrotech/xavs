    ...
    foreach my $j (0 .. scalar(@input_files)-1)
    {
        ...
        foreach my $k (0 .. scalar(@versions)-1)
        {
            ...
            else
            {
                ...
                system("test/xavs-$version/xavs --verbose $opt $file -o $outfile-$version.avs > $outfile-$version.log 2>&1");
            }
            ...
            if ($k > 0)
            {
                ...
                $is_diff ||= compare_rawavs("$outfile-$version.avs", "$outfile-$baseversion.avs");
...
sub parse_log_overall_stats
{
    ...
    if ($logtext !~ m!xavs \[info\]: PSNR Mean Y:(\d+\.\d+) U:(\d+\.\d+) V:(\d+\.\d+) Avg:(\d+\.\d+) Global:(\d+\.\d+) kb/s:(\d+\.\d+)!)
    {
