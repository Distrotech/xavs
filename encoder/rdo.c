...
#define xavs_macroblock_write_cavlc  static xavs_macroblock_size_cavlc
#include "cavlc.c"

/* CABAC: not exactly the same. xavs_cabac_size_decision() keeps track of
 * fractional bits, but only finite precision. */
#undef  xavs_cabac_encode_decision
#undef  xavs_cabac_encode_decision_noup
#define xavs_cabac_encode_decision(c,x,v) xavs_cabac_size_decision(c,x,v)
#define xavs_cabac_encode_decision_noup(c,x,v) xavs_cabac_size_decision_noup(c,x,v)
#define xavs_cabac_encode_terminal(c)     ((c)->f8_bits_encoded += 7)
#define xavs_cabac_encode_bypass(c,v)     ((c)->f8_bits_encoded += 256)
#define xavs_cabac_encode_ue_bypass(c,e,v) ((c)->f8_bits_encoded += (bs_size_ue_big(v+(1<<e)-1)-e)<<8)
#define xavs_macroblock_write_cabac  static xavs_macroblock_size_cabac
#include "cabac.c"

#define COPY_CABAC h->mc.memcpy_aligned( &cabac_tmp.f8_bits_encoded, &h->cabac.f8_bits_encoded, \
        sizeof(xavs_cabac_t) - offsetof(xavs_cabac_t,f8_bits_encoded) )


/* Sum the cached SATDs to avoid repeating them. */
static inline int sum_satd( xavs_t *h, int pixel, int x, int y )
{
    int satd = 0;
    int min_x = x>>2;
    int min_y = y>>2;
    int max_x = (x>>2) + (xavs_pixel_size[pixel].w>>2);
    int max_y = (y>>2) + (xavs_pixel_size[pixel].h>>2);
...
static inline int sum_sa8d( xavs_t *h, int pixel, int x, int y )
{
    int sa8d = 0;
    int min_x = x>>3;
    int min_y = y>>3;
    int max_x = (x>>3) + (xavs_pixel_size[pixel].w>>3);
    int max_y = (y>>3) + (xavs_pixel_size[pixel].h>>3);
    if( pixel == PIXEL_16x16 )
...
static inline int ssd_plane( xavs_t *h, int size, int p, int x, int y )
{
...
static inline int ssd_mb( xavs_t *h )
{
...
static int xavs_rd_cost_mb( xavs_t *h, int i_lambda2 )
{
    ...
    {
        xavs_cabac_t cabac_tmp;
        COPY_CABAC;
        xavs_macroblock_size_cabac( h, &cabac_tmp );
...
static uint64_t xavs_rd_cost_subpart( xavs_t *h, int i_lambda2, int i4, int i_pixel )
{
    uint64_t i_ssd, i_bits;

    xavs_macroblock_encode_p4x4( h, i4 );
    if( i_pixel == PIXEL_8x4 )
        xavs_macroblock_encode_p4x4( h, i4+1 );
    if( i_pixel == PIXEL_4x8 )
        xavs_macroblock_encode_p4x4( h, i4+2 );
    ...
    if( h->param.b_cabac )
    {
        xavs_cabac_t cabac_tmp;
        COPY_CABAC;
        xavs_subpartition_size_cabac( h, &cabac_tmp, i4, i_pixel );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
    {
        i_bits = xavs_subpartition_size_cavlc( h, i4, i_pixel );
    }

    return (i_ssd<<8) + i_bits;
}

uint64_t xavs_rd_cost_part( xavs_t *h, int i_lambda2, int i4, int i_pixel )
{
    uint64_t i_ssd, i_bits;
    int i8 = i4 >> 2;
    int chromassd;
    ...
    if( i_pixel > PIXEL_8x8 )
        return xavs_rd_cost_subpart( h, i_lambda2, i4, i_pixel );
    ...
    if( h->param.b_cabac )
    {
        xavs_cabac_t cabac_tmp;
        COPY_CABAC;
        xavs_partition_size_cabac( h, &cabac_tmp, i8, i_pixel );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
    {
...
static uint64_t xavs_rd_cost_i8x8( xavs_t *h, int i_lambda2, int i8, int i_mode )
{
    uint64_t i_ssd, i_bits;
    ...
    if( h->param.b_cabac )
    {
        xavs_cabac_t cabac_tmp;
        COPY_CABAC;
        xavs_partition_i8x8_size_cabac( h, &cabac_tmp, i8, i_mode );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
    {
...
static uint64_t xavs_rd_cost_i4x4( xavs_t *h, int i_lambda2, int i4, int i_mode )
{
    uint64_t i_ssd, i_bits;

    //xavs_mb_encode_i4x4( h, i4, h->mb.i_qp );
    ...
    if( h->param.b_cabac )
    {
        xavs_cabac_t cabac_tmp;
        COPY_CABAC;
        xavs_partition_i4x4_size_cabac( h, &cabac_tmp, i4, i_mode );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
    {
...
static uint64_t xavs_rd_cost_i8x8_chroma( xavs_t *h, int i_lambda2, int i_mode, int b_dct )
{
    uint64_t i_ssd, i_bits;
    ...
    if( h->param.b_cabac )
    {
        xavs_cabac_t cabac_tmp;
        COPY_CABAC;
        xavs_i8x8_chroma_size_cabac( h, &cabac_tmp );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
    {
    ...
    for( i_prefix = 0; i_prefix < 15; i_prefix++ )
    {
        for( i_ctx = 0; i_ctx < 128; i_ctx++ )
        {
            ...
            for( i = 1; i < i_prefix; i++ )
                f8_bits += xavs_cabac_size_decision2( &ctx, 1 );
            if( i_prefix > 0 && i_prefix < 14 )
                f8_bits += xavs_cabac_size_decision2( &ctx, 0 );
...
// use trellis' RD score instead of xavs_mb_decimate_score?
...
static ALWAYS_INLINE int quant_trellis_cabac( xavs_t *h, int16_t *dct,
                                 ...
                                 int i_ctxBlockCat, int i_lambda2, int b_ac, int dc, int i_coefs, int idx )
{
    ...
    for( i = i_last_nnz; i >= b_ac; i-- )
    {
        ...
        if( q == 0 )
        {
            ...
            const uint32_t cost_sig0 = xavs_cabac_size_decision_noup2( &cabac_state_sig[sigindex], 0 )
...
#define SET_LEVEL(n,l) \
        ...
        if( i < i_coefs-1 )
        {
            ...
            cost_sig[0] = xavs_cabac_size_decision_noup2( &cabac_state_sig[sigindex], 0 );
            cost_sig[1] = xavs_cabac_size_decision_noup2( &cabac_state_sig[sigindex], 1 );
            cost_last[0] = xavs_cabac_size_decision_noup2( &cabac_state_last[lastindex], 0 );
            cost_last[1] = xavs_cabac_size_decision_noup2( &cabac_state_last[lastindex], 1 );
        }
        else
        {
        ...
        for( abs_level = q; abs_level >= q-1; abs_level-- )
        {
            ...
            for( j = 0; j < 8; j++ )
            {
                ...
                if( abs_level || node_ctx )
                {
                    ...
                    if( abs_level )
                    {
                        const int i_prefix = XAVS_MIN( abs_level - 1, 14 );
                        f8_bits += cost_last[ node_ctx == 0 ];
                        f8_bits += xavs_cabac_size_decision2( &n.cabac_state[coeff_abs_level1_ctx[node_ctx]], i_prefix > 0 );
                        if( i_prefix > 0 )
                        {
...
const static uint8_t xavs_zigzag_scan2[4] = {0,1,2,3};
