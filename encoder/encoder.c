...
static void xavs_slice_header_write( bs_t *s, xavs_slice_header_t *sh, 
...
xavs_t *xavs_encoder_open   ( xavs_param_t *param )
{
    ...
    xavs_log( h, XAVS_LOG_INFO, "using cpu capabilities %s%s%s%s%s%s\n",
             param->cpu&XAVS_CPU_MMX ? "MMX " : "",
             param->cpu&XAVS_CPU_MMXEXT ? "MMXEXT " : "",
             param->cpu&XAVS_CPU_SSE ? "SSE " : "",
             param->cpu&XAVS_CPU_SSE2 ? "SSE2 " : "",
...
/* internal usage */
static void xavs_nal_start( xavs_t *h, int i_type, int i_ref_idc )
{
...
static void xavs_nal_end( xavs_t *h )
{
    xavs_nal_t *nal = &h->out.nal[h->out.i_nal];
...
static void xavs_frame_put( xavs_frame_t *list[XAVS_BFRAME_MAX], xavs_frame_t *frame )
{
    int i = 0;
    while( list[i] ) i++;
    list[i] = frame;
...
static void xavs_frame_push( xavs_frame_t *list[XAVS_BFRAME_MAX], xavs_frame_t *frame )
{
    int i = 0;
    while( list[i] ) i++;
    while( i-- )
...
static xavs_frame_t *xavs_frame_get( xavs_frame_t *list[XAVS_BFRAME_MAX+1] )
{
...
static void xavs_frame_sort( xavs_frame_t *list[XAVS_BFRAME_MAX+1], int b_dts )
{
    int i, b_ok;
    do {
        b_ok = 1;
...
static inline void xavs_reference_build_list( xavs_t *h, int i_poc, int i_slice_type )
{
    int i;
    int b_ok;
...
static inline void xavs_reference_update( xavs_t *h )
{
    int i;
...
static void xavs_pb_pic_header_init(xavs_t *h, xavs_pb_pic_header_t *pbh, int i_qp, int i_slice_type)
{
...
static inline int xavs_slices_write( xavs_t *h )
{
    int i_frame_size;
        ...
        if( h->stat.i_slice_count[i_slice] > 0 )
        {
            ...
            if( h->param.analyse.b_psnr )
            {
                xavs_log( h, XAVS_LOG_INFO,
                          ...
                          xavs_psnr( h->stat.i_sqe_global[i_slice], i_count * i_yuv_size ) );
            }
            else
            {
                xavs_log( h, XAVS_LOG_INFO,
...
#define SUM3b(p,o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
    ...
    for( i = 0; i < XAVS_BFRAME_MAX + 3; i++ )
    {
        if( h->frames.current[i] ) xavs_frame_delete( h->frames.current[i] );
        if( h->frames.next[i] )    xavs_frame_delete( h->frames.next[i] );
        if( h->frames.unused[i] )  xavs_frame_delete( h->frames.unused[i] );
    }
    /* ref frames */
    for( i = 0; i < h->frames.i_max_dpb; i++ )
    {
