...
static void xavs_mb_analyse_load_costs( xavs_t *h, xavs_mb_analysis_t *a )
{
...
static void predict_8x8_mode_available( unsigned int i_neighbour,
...
static void xavs_mb_analyse_intra( xavs_t *h, xavs_mb_analysis_t *a, int i_cost_inter )
{
        ...
        for( idx = 0; idx < 4; idx++ )
        {
            ...
            for( i = 0; i < i_max; i++ )
            {
                ...
                i_sad = h->pixf.mbcmp[PIXEL_8x8]( p_dst_by, FDEC_STRIDE,
                      ...
                      + a->i_lambda * (i_pred_mode == xavs_mb_pred_mode8x8(i_mode) ? 1 : 4);
...
static void xavs_mb_analyse_inter_p4x4( xavs_t *h, xavs_mb_analysis_t *a, int i8x8 )
{
    ...
    for( i4x4 = 0; i4x4 < 4; i4x4++ )
    {
        ...
        xavs_macroblock_cache_mv( h, x4, y4, 1, 1, 0, m->mv[0], m->mv[1] );
    }
...
static void xavs_mb_analyse_inter_p8x4( xavs_t *h, xavs_mb_analysis_t *a, int i8x8 )
{
    ...
    for( i8x4 = 0; i8x4 < 2; i8x4++ )
    {
        ...
        xavs_macroblock_cache_mv( h, x4, y4, 2, 1, 0, m->mv[0], m->mv[1] );
    }
...
static void xavs_mb_analyse_inter_p4x8( xavs_t *h, xavs_mb_analysis_t *a, int i8x8 )
{
    ...
    for( i4x8 = 0; i4x8 < 2; i4x8++ )
    {
        ...
        xavs_macroblock_cache_mv( h, x4, y4, 1, 2, 0, m->mv[0], m->mv[1] );
    }
...
static void xavs_mb_analyse_inter_b8x8( xavs_t *h, xavs_mb_analysis_t *a )
{
    ...
    for( i = 0; i < 4; i++ )
    {
		...
		if( h->mb.i_sub_partition[i] == D_L0_8x8)
		{
			...
			xavs_macroblock_cache_mv(h, 2*x8, 2*y8, 2, 2, 0, a->l0.me8x8[i].mv[0],
				                                             a->l0.me8x8[i].mv[1] );
		...
		if( h->mb.i_sub_partition[i] == D_L1_8x8)
		{
			...
			xavs_macroblock_cache_mv(h, 2*x8, 2*y8, 2, 2, 1, a->l1.me8x8[i].mv[0], 
				                                             a->l1.me8x8[i].mv[1]);
		}
...
static void xavs_mb_analyse_inter_b16x8( xavs_t *h, xavs_mb_analysis_t *a )
{
    ...
    for( i = 0; i < 2; i++ )
    {
		...
		if( a->i_mb_partition16x8[i] == D_L0_8x8)
		{
			...
			xavs_macroblock_cache_mv(h, 0, 2*i, 4, 2, 0, a->l0.me16x8[i].mv[0],
				                                         a->l0.me16x8[i].mv[1]);				                                            
		...
		if( a->i_mb_partition16x8[i] == D_L1_8x8)
		{
			...
			xavs_macroblock_cache_mv(h, 0, 2*i, 4, 2, 1, a->l1.me16x8[i].mv[0], 
				                                         a->l1.me16x8[i].mv[1]);
		}
...
static void xavs_mb_analyse_inter_b8x16( xavs_t *h, xavs_mb_analysis_t *a )
{
    ...
    for( i = 0; i < 2; i++ )
    {
		...
		if( a->i_mb_partition16x8[i] == D_L0_8x8)
		{
			...
			xavs_macroblock_cache_mv(h, 2*i, 0, 2, 4, 0, a->l0.me8x16[i].mv[0],
				                                         a->l0.me8x16[i].mv[1]);				                                            
		...
		if( a->i_mb_partition16x8[i] == D_L1_8x8)
		{
			...
			xavs_macroblock_cache_mv(h, 2*i, 0, 2, 4, 1, a->l1.me8x16[i].mv[0], 
				                                         a->l1.me8x16[i].mv[1]);
		}
		if( a->i_mb_partition16x8[i] == D_BI_8x8)
		{
...
*/
static void refine_bidir( xavs_t *h, xavs_mb_analysis_t *a )
{
...
static inline void xavs_mb_analyse_transform( xavs_t *h )
{
    h->mb.cache.b_transform_8x8_allowed =
        ...
        && !IS_INTRA( h->mb.i_type ) && xavs_mb_transform_8x8_allowed( h );
...
static inline void xavs_mb_analyse_transform_rd( xavs_t *h, xavs_mb_analysis_t *a, int *i_cost )
{
    h->mb.cache.b_transform_8x8_allowed =
        h->param.analyse.b_transform_8x8 && xavs_mb_transform_8x8_allowed( h );
        ...
        else
        {
                ...
                analysis.l0.i_cost8x8 < analysis.l0.me16x16.cost )
            {
                ...
                if( flags & XAVS_ANALYSE_PSUB8x8 )
                {
                    ...
                    {
                        i_cost = xavs_rd_cost_mb( h, analysis.i_lambda2 );
...
static void xavs_analyse_update_cache( xavs_t *h, xavs_mb_analysis_t *a  )
{
    ...
    switch( h->mb.i_type )
    {
        case I_4x4:
            for( i = 0; i < 16; i++ )
            {
                h->mb.cache.intra4x4_pred_mode[xavs_scan8[i]] =
        ...
        case I_8x8:
            for( i = 0; i < 4; i++ )
                xavs_macroblock_cache_intra8x8_pred( h, 2*(i&1), 2*(i>>1),
        ...
        case B_8x8:
            ...
            for( i = 0; i < 4; i++ )
			{
	             if( h->mb.i_sub_partition[i] == D_L0_8x8)
				 {
			         ...
			         xavs_macroblock_cache_mv(h, (i%2)*2, (i/2)*2, 2, 2, 0, a->l0.me8x8[i].mv[0],
				                                                            a->l0.me8x8[i].mv[1] );
		         ...
		         if( h->mb.i_sub_partition[i] == D_L1_8x8)
				 {
			         ...
			         xavs_macroblock_cache_mv(h, (i%2)*2, (i/2)*2, 2, 2, 1, a->l1.me8x8[i].mv[0], 
				                                                      a->l1.me8x8[i].mv[1]);
				 }
