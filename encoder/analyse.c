...
#ifdef _MSC_VER
#define log2f(x) (log(x)*1.4426950408889634f)
#endif
...
static const int xavs_trellis_lambda2_tab[2][52] = {
...
static const uint16_t xavs_chroma_lambda2_offset_tab[] = {
       16,    20,    25,    32,    40,    50,
...
static const int i_mb_b_cost_table[XAVS_MBTYPE_MAX] = {
    9, 9, 0, 0, 0, 1, 3, 7, 7, 7, 3, 7, 7, 7, 5, 9, 0
};
...
uint16_t *xavs_cost_mv_fpel[92][4];
uint16_t xavs_cost_ref[92][3][33];

/* initialize an array of lambda*nbits for all possible mvs */
static int xavs_mb_analyse_load_costs( xavs_t *h, xavs_mb_analysis_t *a )
{
    ...
    if( !p_cost_mv[a->i_lambda] )
    {
        ...
        for( i = 0; i < 3; i++ )
            for( j = 0; j < 33; j++ )
                xavs_cost_ref[a->i_lambda][i][j] = i ? a->i_lambda * bs_size_te( i, j ) : 0;
    }
    a->p_cost_mv = p_cost_mv[a->i_lambda];
    a->p_cost_ref0 = xavs_cost_ref[a->i_lambda][xavs_clip3(h->sh.i_num_ref_idx_l0_active-1,0,2)];
    a->p_cost_ref1 = xavs_cost_ref[a->i_lambda][xavs_clip3(h->sh.i_num_ref_idx_l1_active-1,0,2)];

    /* FIXME is this useful for all me methods? */
    if( h->param.analyse.i_me_method >= XAVS_ME_ESA && !xavs_cost_mv_fpel[a->i_lambda][0] )
    {
        for( j=0; j<4; j++ )
        {
            CHECKED_MALLOC( xavs_cost_mv_fpel[a->i_lambda][j], (4*2048 + 1) * sizeof(int16_t) );
            xavs_cost_mv_fpel[a->i_lambda][j] += 2*2048;
            for( i = -2*2048; i < 2*2048; i++ )
                xavs_cost_mv_fpel[a->i_lambda][j][i] = p_cost_mv[a->i_lambda][i*4+j];
        }
    }
    return 0;
fail:
...
static void xavs_mb_analyse_init( xavs_t *h, xavs_mb_analysis_t *a, int i_qp )
{
    ...
    a->i_lambda = xavs_lambda_tab[i_qp];
    a->i_lambda2 = xavs_lambda2_tab[i_qp];

    h->mb.b_trellis = h->param.analyse.i_trellis > 1 && a->i_mbrd;
    if( h->param.analyse.i_trellis )
    {
        h->mb.i_trellis_lambda2[0][0] = xavs_trellis_lambda2_tab[0][h->mb.i_qp];
        h->mb.i_trellis_lambda2[0][1] = xavs_trellis_lambda2_tab[1][h->mb.i_qp];
        h->mb.i_trellis_lambda2[1][0] = xavs_trellis_lambda2_tab[0][h->mb.i_chroma_qp];
        h->mb.i_trellis_lambda2[1][1] = xavs_trellis_lambda2_tab[1][h->mb.i_chroma_qp];
    }
    h->mb.i_psy_rd_lambda = a->i_lambda;
    /* Adjusting chroma lambda based on QP offset hurts PSNR but improves visual quality. */
    h->mb.i_chroma_lambda2_offset = h->param.analyse.b_psy ? xavs_chroma_lambda2_offset_tab[h->mb.i_qp-h->mb.i_chroma_qp+12] : 256;
    ...
    a->i_satd_pcm = !h->mb.i_psy_rd && a->i_mbrd ? ((uint64_t)XAVS_PCM_COST*a->i_lambda2 + 128) >> 8 : COST_MAX;
...
#define CLIP_FMV(mv) xavs_clip3( mv, -i_fmv_range, i_fmv_range-1 )
        ...
        if( h->mb.i_mb_x == 0)
        {
            ...
            if( h->param.i_threads > 1 )
            {
                ...
                {
                    xavs_frame_t **fref = i ? h->fref1 : h->fref0;
                    int i_ref = i ? h->i_ref1 : h->i_ref0;
                    for( j=0; j<i_ref; j++ )
                    {
                        xavs_frame_cond_wait( fref[j], thresh );
                        thread_mvy_range = XAVS_MIN( thread_mvy_range, fref[j]->i_lines_completed - pix_y );
                    }
                }
            ...
            h->mb.mv_min_spel[1] = xavs_clip3( h->mb.mv_min[1], -i_fmv_range, i_fmv_range );
            h->mb.mv_max_spel[1] = CLIP_FMV( h->mb.mv_max[1] );
            h->mb.mv_max_spel[1] = XAVS_MIN( h->mb.mv_max_spel[1], thread_mvy_range*4 );
...
#undef CLIP_FMV
        ...
        if( h->mb.i_mb_xy - h->sh.i_first_mb > 4 )
        {
            if(   IS_INTRA( h->mb.i_mb_type_left )
               ...
               || (h->mb.i_mb_xy - h->sh.i_first_mb < 3*(h->stat.frame.i_mb_count[I_8x8] + h->stat.frame.i_mb_count[I_16x16])) )
            { /* intra is likely */ }
            else
            {
                a->b_fast_intra = 1;
...
static void inline xavs_psy_trellis_init( xavs_t *h, int do_both_dct )
{
...
static inline void xavs_mb_cache_fenc_satd( xavs_t *h )
{
    DECLARE_ALIGNED_16( static uint8_t zero[16] ) = {0};
    uint8_t *fenc;
    int x, y, satd_sum = 0, sa8d_sum = 0;
    if( h->param.analyse.i_trellis == 2 && h->mb.i_psy_trellis )
        xavs_psy_trellis_init( h, h->param.analyse.b_transform_8x8 );
    if( !h->mb.i_psy_rd )
        return;
    for( y = 0; y < 4; y++ )
        for( x = 0; x < 4; x++ )
...
static void xavs_mb_analyse_intra_chroma( xavs_t *h, xavs_mb_analysis_t *a )
{
	int i,ii,jj;
	...
	for( i=0; i<i_max; i++ )
	{
		...
		sadCr = h->pixf.mbcmp[PIXEL_8x8]( p_dstc[1], FDEC_STRIDE,
			p_srcc[1], FENC_STRIDE ); 
		i_satd = sadCb + sadCr +
			a->i_lambda * bs_size_ue( xavs_mb_pred_mode8x8c_fix[i_mode] );
...
static void xavs_mb_analyse_intra( xavs_t *h, xavs_mb_analysis_t *a, int i_satd_inter )
{
...
static void xavs_intra_rd( xavs_t *h, xavs_mb_analysis_t *a, int i_satd_thresh )
{
    if( a->i_satd_i16x16 <= i_satd_thresh )
    {
        ...
        a->i_satd_i16x16 = xavs_rd_cost_mb( h, a->i_lambda2 );
    }
    else
    ...
    if( a->i_satd_i8x8 <= i_satd_thresh && a->i_satd_i8x8 < COST_MAX )
    {
        h->mb.i_type = I_8x8;
        xavs_analyse_update_cache( h, a );
        a->i_satd_i8x8 = xavs_rd_cost_mb( h, a->i_lambda2 );
...
static void xavs_intra_rd_refine( xavs_t *h, xavs_mb_analysis_t *a )
{
    ...
    if( h->mb.i_type == I_16x16 )
    {
        ...
        for( i = 0; i < i_max; i++ )
        {
            ...
            i_satd = xavs_rd_cost_mb( h, a->i_lambda2 );
    ...
    if( i_max > 1 )
    {
        ...
        if( i_max > 0 )
        {
            ...
            int i_chroma_lambda = xavs_lambda2_tab[h->mb.i_chroma_qp];
            /* the previous thing encoded was xavs_intra_rd(), so the pixels and
        ...
        for( idx = 0; idx < 4; idx++ )
        {
            ...
            for( i = 0; i < i_max; i++ )
            {
                ...
                if( i_best > i_satd )
                {
                    ...
                    i_nnz[0] = *(uint16_t*)&h->mb.cache.non_zero_count[xavs_scan8[4*idx+0]];
                    i_nnz[1] = *(uint16_t*)&h->mb.cache.non_zero_count[xavs_scan8[4*idx+2]];
                }
            }
            ...
            *(uint16_t*)&h->mb.cache.non_zero_count[xavs_scan8[4*idx+0]] = i_nnz[0];
            *(uint16_t*)&h->mb.cache.non_zero_count[xavs_scan8[4*idx+2]] = i_nnz[1];
...
static void xavs_mb_analyse_inter_p16x16( xavs_t *h, xavs_mb_analysis_t *a )
{
    xavs_me_t m;
    int i_ref, i_mvc;
    ...
    for( i_ref = 0; i_ref < h->mb.pic.i_fref[0]; i_ref++ )
    {
        ...
        if( m.cost < a->l0.me16x16.cost )
            h->mc.memcpy_aligned( &a->l0.me16x16, &m, sizeof(xavs_me_t) );
    ...
    if( a->i_mbrd )
    {
        ...
        if( a->l0.me16x16.i_ref == 0 && *(uint32_t*)a->l0.me16x16.mv == *(uint32_t*)h->mb.cache.pskip_mv )
        {
            ...
            xavs_macroblock_cache_mv_ptr( h, 0, 0, 4, 4, 0, a->l0.me16x16.mv );
            a->l0.i_rd16x16 = xavs_rd_cost_mb( h, a->i_lambda2 );
        }
    }
}

static void xavs_mb_analyse_inter_p8x8_mixed_ref( xavs_t *h, xavs_mb_analysis_t *a )
{
    xavs_me_t m;
    int i_ref;
    ...
    for( i = 0; i < 4; i++ )
    {
        ...
        for( i_ref = 0; i_ref <= i_maxref; i_ref++ )
        {
            ...
            *(uint32_t*)a->l0.mvc[i_ref][i+1] = *(uint32_t*)m.mv;
            ...
            if( m.cost < l0m->cost )
                h->mc.memcpy_aligned( l0m, &m, sizeof(xavs_me_t) );
        }
        xavs_macroblock_cache_mv_ptr( h, 2*x8, 2*y8, 2, 2, 0, l0m->mv );
...
static void xavs_mb_analyse_inter_p8x8( xavs_t *h, xavs_mb_analysis_t *a )
{
    ...
    *(uint32_t*)mvc[0] = *(uint32_t*)a->l0.me16x16.mv;
    ...
    for( i = 0; i < 4; i++ )
    {
        ...
        xavs_macroblock_cache_mv_ptr( h, 2*x8, 2*y8, 2, 2, 0, m->mv );
...
static void xavs_mb_analyse_inter_p16x8( xavs_t *h, xavs_mb_analysis_t *a )
{
    xavs_me_t m;
    uint8_t  **p_fenc = h->mb.pic.p_fenc;
    ...
    for( i = 0; i < 2; i++ )
    {
        ...
        xavs_macroblock_cache_mv_ptr( h, 0, 2*i, 4, 2, 0, l0m->mv );
...
static void xavs_mb_analyse_inter_p8x16( xavs_t *h, xavs_mb_analysis_t *a )
{
    xavs_me_t m;
    uint8_t  **p_fenc = h->mb.pic.p_fenc;
    DECLARE_ALIGNED_4( int16_t mvc[3][2] );
    ...
    for( i = 0; i < 2; i++ )
    {
        ...
        xavs_macroblock_cache_mv_ptr( h, 2*i, 0, 2, 4, 0, l0m->mv );
...
static int xavs_mb_analyse_inter_p4x4_chroma( xavs_t *h, xavs_mb_analysis_t *a, uint8_t **p_fref, int i8x8, int pixel )
{
...
static void xavs_mb_analyse_inter_p4x4( xavs_t *h, xavs_mb_analysis_t *a, int i8x8 )
{
    ...
    for( i4x4 = 0; i4x4 < 4; i4x4++ )
    {
        ...
        xavs_me_t *m = &a->l0.me4x4[i8x8][i4x4];
        ...
        xavs_mb_predict_mv( h, 0, idx, 1, m->mvp );
        xavs_me_search( h, m, &a->l0.me8x8[i8x8].mv, i_mvc );

        xavs_macroblock_cache_mv_ptr( h, x4, y4, 1, 1, 0, m->mv );
    }
    ...
    if( h->mb.b_chroma_me )
        a->l0.i_cost4x4[i8x8] += xavs_mb_analyse_inter_p4x4_chroma( h, a, p_fref, i8x8, PIXEL_4x4 );
}

static void xavs_mb_analyse_inter_p8x4( xavs_t *h, xavs_mb_analysis_t *a, int i8x8 )
{
    uint8_t  **p_fref = h->mb.pic.p_fref[0][a->l0.me8x8[i8x8].i_ref];
    uint8_t  **p_fenc = h->mb.pic.p_fenc;
    const int i_ref = a->l0.me8x8[i8x8].i_ref;
    ...
    for( i8x4 = 0; i8x4 < 2; i8x4++ )
    {
        ...
        xavs_me_t *m = &a->l0.me8x4[i8x8][i8x4];
        ...
        xavs_mb_predict_mv( h, 0, idx, 2, m->mvp );
        xavs_me_search( h, m, &a->l0.me4x4[i8x8][0].mv, i_mvc );

        xavs_macroblock_cache_mv_ptr( h, x4, y4, 2, 1, 0, m->mv );
    }
    a->l0.i_cost8x4[i8x8] = a->l0.me8x4[i8x8][0].cost + a->l0.me8x4[i8x8][1].cost +
                            REF_COST( 0, i_ref ) +
                            a->i_lambda * i_sub_mb_p_cost_table[D_L0_8x4];
    if( h->mb.b_chroma_me )
        a->l0.i_cost8x4[i8x8] += xavs_mb_analyse_inter_p4x4_chroma( h, a, p_fref, i8x8, PIXEL_8x4 );
}

static void xavs_mb_analyse_inter_p4x8( xavs_t *h, xavs_mb_analysis_t *a, int i8x8 )
{
    uint8_t  **p_fref = h->mb.pic.p_fref[0][a->l0.me8x8[i8x8].i_ref];
    uint8_t  **p_fenc = h->mb.pic.p_fenc;
    const int i_ref = a->l0.me8x8[i8x8].i_ref;
    ...
    for( i4x8 = 0; i4x8 < 2; i4x8++ )
    {
        ...
        xavs_me_t *m = &a->l0.me4x8[i8x8][i4x8];
        ...
        xavs_macroblock_cache_mv_ptr( h, x4, y4, 1, 2, 0, m->mv );
    }
    a->l0.i_cost4x8[i8x8] = a->l0.me4x8[i8x8][0].cost + a->l0.me4x8[i8x8][1].cost +
                            REF_COST( 0, i_ref ) +
                            a->i_lambda * i_sub_mb_p_cost_table[D_L0_4x8];
    if( h->mb.b_chroma_me )
        a->l0.i_cost4x8[i8x8] += xavs_mb_analyse_inter_p4x4_chroma( h, a, p_fref, i8x8, PIXEL_4x8 );
}

static void xavs_mb_analyse_inter_direct( xavs_t *h, xavs_mb_analysis_t *a )
{
...
static void xavs_mb_analyse_inter_b16x16( xavs_t *h, xavs_mb_analysis_t *a )
{
    ...
    for( i_ref = 0; i_ref < h->mb.pic.i_fref[1]; i_ref++ )
    {
        ...
        {
            a->l1.i_ref = i_ref;
            h->mc.memcpy_aligned( &a->l1.me16x16, &m, sizeof(xavs_me_t) );
        }
...
static inline void xavs_mb_cache_mv_p8x8( xavs_t *h, xavs_mb_analysis_t *a, int i )
{
    ...
    {
        case D_L0_8x8:
            xavs_macroblock_cache_mv_ptr( h, x, y, 2, 2, 0, a->l0.me8x8[i].mv );
            break;
        case D_L0_8x4:
            xavs_macroblock_cache_mv_ptr( h, x, y+0, 2, 1, 0, a->l0.me8x4[i][0].mv );
            xavs_macroblock_cache_mv_ptr( h, x, y+1, 2, 1, 0, a->l0.me8x4[i][1].mv );
            break;
        case D_L0_4x8:
            xavs_macroblock_cache_mv_ptr( h, x+0, y, 1, 2, 0, a->l0.me4x8[i][0].mv );
            xavs_macroblock_cache_mv_ptr( h, x+1, y, 1, 2, 0, a->l0.me4x8[i][1].mv );
            break;
        case D_L0_4x4:
            xavs_macroblock_cache_mv_ptr( h, x+0, y+0, 1, 1, 0, a->l0.me4x4[i][0].mv );
            xavs_macroblock_cache_mv_ptr( h, x+1, y+0, 1, 1, 0, a->l0.me4x4[i][1].mv );
            xavs_macroblock_cache_mv_ptr( h, x+0, y+1, 1, 1, 0, a->l0.me4x4[i][2].mv );
            xavs_macroblock_cache_mv_ptr( h, x+1, y+1, 1, 1, 0, a->l0.me4x4[i][3].mv );
            break;
        default:
...
#define CACHE_MV_BI(x,y,dx,dy,me0,me1,part) \
    ...
    { \
        ...
        xavs_macroblock_cache_mv_ptr( h, x,y,dx,dy, 0, me0.mv ); \
    } \
    else \
    { \
        xavs_macroblock_cache_ref( h, x,y,dx,dy, 0, -1 ); \
        xavs_macroblock_cache_mv(  h, x,y,dx,dy, 0, 0 ); \
        if( b_mvd ) \
            xavs_macroblock_cache_mvd( h, x,y,dx,dy, 0, 0 ); \
    } \
    if( xavs_mb_partition_listX_table[1][part] ) \
    { \
        xavs_macroblock_cache_ref( h, x,y,dx,dy, 1, a->l1.i_ref ); \
        xavs_macroblock_cache_mv_ptr( h, x,y,dx,dy, 1, me1.mv ); \
    } \
    else \
    { \
        xavs_macroblock_cache_ref( h, x,y,dx,dy, 1, -1 ); \
        xavs_macroblock_cache_mv(  h, x,y,dx,dy, 1, 0 ); \
        if( b_mvd ) \
            xavs_macroblock_cache_mvd( h, x,y,dx,dy, 1, 0 ); \
    }

static inline void xavs_mb_cache_mv_b8x8( xavs_t *h, xavs_mb_analysis_t *a, int i, int b_mvd )
{
    int x = (i%2)*2;
    int y = (i/2)*2;
    if( h->mb.i_sub_partition[i] == D_DIRECT_8x8 )
    {
        ...
        if( b_mvd )
        {
            xavs_macroblock_cache_mvd(  h, x, y, 2, 2, 0, 0 );
            xavs_macroblock_cache_mvd(  h, x, y, 2, 2, 1, 0 );
...
static inline void xavs_mb_cache_mv_b16x8( xavs_t *h, xavs_mb_analysis_t *a, int i, int b_mvd )
{
    CACHE_MV_BI( 0, 2*i, 4, 2, a->l0.me16x8[i], a->l1.me16x8[i], a->i_mb_partition16x8[i] );
}
static inline void xavs_mb_cache_mv_b8x16( xavs_t *h, xavs_mb_analysis_t *a, int i, int b_mvd )
{
    CACHE_MV_BI( 2*i, 0, 2, 4, a->l0.me8x16[i], a->l1.me8x16[i], a->i_mb_partition8x16[i] );
}
#undef CACHE_MV_BI

static void xavs_mb_analyse_inter_b8x8( xavs_t *h, xavs_mb_analysis_t *a )
{
    uint8_t **p_fref[2] =
    ...
    for( i = 0; i < 4; i++ )
    {
        ...
        for( l = 0; l < 2; l++ )
        {
            ...
            xavs_macroblock_cache_mv_ptr( h, 2*x8, 2*y8, 2, 2, l, m->mv );
...
static void xavs_mb_analyse_inter_b16x8( xavs_t *h, xavs_mb_analysis_t *a )
{
    uint8_t **p_fref[2] =
        { h->mb.pic.p_fref[0][a->l0.i_ref],
          h->mb.pic.p_fref[1][a->l1.i_ref] };
    ...
    for( i = 0; i < 2; i++ )
    {
        ...
        for( l = 0; l < 2; l++ )
        {
            ...
            *(uint32_t*)mvc[1] = *(uint32_t*)lX->me8x8[2*i+1].mv;
            ...
            xavs_mb_predict_mv( h, l, 8*i, 2, m->mvp );
...
static void xavs_mb_analyse_inter_b8x16( xavs_t *h, xavs_mb_analysis_t *a )
{
    uint8_t **p_fref[2] =
        { h->mb.pic.p_fref[0][a->l0.i_ref],
          h->mb.pic.p_fref[1][a->l1.i_ref] };
...
static void xavs_mb_analyse_p_rd( xavs_t *h, xavs_mb_analysis_t *a, int i_satd )
{
    ...
    if( a->l0.i_cost8x8 <= thresh )
    {
        h->mb.i_type = P_8x8;
        h->mb.i_partition = D_8x8;
        if( h->param.analyse.inter & XAVS_ANALYSE_PSUB8x8 )
        {
            int i;
            ...
            for( i = 0; i < 4; i++ )
            {
                ...
                int thresh = XAVS_MIN4( costs[0], costs[1], costs[2], costs[3] ) * 5 / 4;
                ...
                for( subtype = D_L0_4x4; subtype <= D_L0_8x8; subtype++ )
                {
                    ...
                    cost = xavs_rd_cost_part( h, a->i_lambda2, i<<2, PIXEL_8x8 );
...
static void xavs_mb_analyse_b_rd( xavs_t *h, xavs_mb_analysis_t *a, int i_satd_inter )
{
    ...
    if( a->b_direct_available && a->i_rd16x16direct == COST_MAX )
    {
        ...
        h->mb.b_skip_mc = 1;
        xavs_analyse_update_cache( h, a );
        a->i_rd16x16direct = xavs_rd_cost_mb( h, a->i_lambda2 );
        h->mb.b_skip_mc = 0;
    }
    ...
    if( a->l1.me16x16.cost <= thresh && a->l1.i_rd16x16 == COST_MAX )
    {
        h->mb.i_type = B_L1_L1;
        xavs_analyse_update_cache( h, a );
        a->l1.i_rd16x16 = xavs_rd_cost_mb( h, a->i_lambda2 );
    }
    ...
    if( a->i_cost16x16bi <= thresh && a->i_rd16x16bi == COST_MAX )
    {
        h->mb.i_type = B_BI_BI;
        xavs_analyse_update_cache( h, a );
        a->i_rd16x16bi = xavs_rd_cost_mb( h, a->i_lambda2 );
    }
    ...
    if( a->i_cost8x8bi <= thresh && a->i_rd8x8bi == COST_MAX )
    {
        ...
        h->mb.i_partition = D_8x8;
        xavs_analyse_update_cache( h, a );
        a->i_rd8x8bi = xavs_rd_cost_mb( h, a->i_lambda2 );
    ...
    if( a->i_cost16x8bi <= thresh && a->i_rd16x8bi == COST_MAX )
    {
        ...
        h->mb.i_partition = D_16x8;
        xavs_analyse_update_cache( h, a );
        a->i_rd16x8bi = xavs_rd_cost_mb( h, a->i_lambda2 );
    }
    ...
    if( a->i_cost8x16bi <= thresh && a->i_rd8x16bi == COST_MAX )
    {
        ...
        h->mb.i_partition = D_8x16;
        xavs_analyse_update_cache( h, a );
        a->i_rd8x16bi = xavs_rd_cost_mb( h, a->i_lambda2 );
    }
}

static void xavs_refine_bidir( xavs_t *h, xavs_mb_analysis_t *a )
{
    ...
    switch( h->mb.i_partition )
    {
        case D_16x16:
            if( h->mb.i_type == B_BI_BI )
                xavs_me_refine_bidir_satd( h, &a->l0.me16x16, &a->l1.me16x16, i_biweight );
            break;
        case D_16x8:
            for( i=0; i<2; i++ )
                if( a->i_mb_partition16x8[i] == D_BI_8x8 )
                    xavs_me_refine_bidir_satd( h, &a->l0.me16x8[i], &a->l1.me16x8[i], i_biweight );
            break;
        case D_8x16:
            for( i=0; i<2; i++ )
                if( a->i_mb_partition8x16[i] == D_BI_8x8 )
                    xavs_me_refine_bidir_satd( h, &a->l0.me8x16[i], &a->l1.me8x16[i], i_biweight );
            break;
        case D_8x8:
            for( i=0; i<4; i++ )
                if( h->mb.i_sub_partition[i] == D_BI_8x8 )
                    xavs_me_refine_bidir_satd( h, &a->l0.me8x8[i], &a->l1.me8x8[i], i_biweight );
            break;
    }
}

static inline void xavs_mb_analyse_transform( xavs_t *h )
{
    if( xavs_mb_transform_8x8_allowed( h ) && h->param.analyse.b_transform_8x8 && !h->mb.b_lossless )
    {
        int i_cost4, i_cost8;
...
static inline void xavs_mb_analyse_transform_rd( xavs_t *h, xavs_mb_analysis_t *a, int *i_satd, int *i_rd )
{
    if( xavs_mb_transform_8x8_allowed( h ) && h->param.analyse.b_transform_8x8 )
    {
        int i_rd8;
        xavs_analyse_update_cache( h, a );
        h->mb.b_transform_8x8 ^= 1;
        /* FIXME only luma is needed, but the score for comparison already includes chroma */
        i_rd8 = xavs_rd_cost_mb( h, a->i_lambda2 );
 ...
 * trick. */
static inline void xavs_mb_analyse_qp_rd( xavs_t *h, xavs_mb_analysis_t *a )
{
    int bcost, cost, direction, failures, prevcost, origcost;
    int orig_qp = h->mb.i_qp, bqp = h->mb.i_qp;
    origcost = bcost = xavs_rd_cost_mb( h, a->i_lambda2 );
    ...
    for( direction = h->mb.cbp[h->mb.i_mb_xy] ? 1 : -1; direction >= -1; direction-=2 )
    {
        ...
        while( h->mb.i_qp > 0 && h->mb.i_qp < 51 )
        {
            ...
            cost = xavs_rd_cost_mb( h, a->i_lambda2 );
    ...
    if( h->mb.i_qp != orig_qp && xavs_mb_transform_8x8_allowed( h ) &&
...
int xavs_macroblock_analyse( xavs_t *h )
{
    ...
    if( h->sh.i_type == SLICE_TYPE_I )
    {
        ...
        if( analysis.i_mbrd )
            xavs_intra_rd( h, &analysis, COST_MAX );
    ...
    else if( h->sh.i_type == SLICE_TYPE_P )
    {
        ...
        else
        {
            ...
            if( xavs_mb_analyse_load_costs( h, &analysis ) )
                return -1;
                ...
                analysis.l0.i_cost8x8 < analysis.l0.me16x16.cost )
            {
                ...
                /* Do sub 8x8 */
                if( flags & XAVS_ANALYSE_PSUB8x8 )
                {
                    for( i = 0; i < 4; i++ )
                    {
                        xavs_mb_analyse_inter_p4x4( h, &analysis, i );
                        if( analysis.l0.i_cost4x4[i] < analysis.l0.me8x8[i].cost )
                        {
                            int i_cost8x8 = analysis.l0.i_cost4x4[i];
                            h->mb.i_sub_partition[i] = D_L0_4x4;

                            xavs_mb_analyse_inter_p8x4( h, &analysis, i );
                            COPY2_IF_LT( i_cost8x8, analysis.l0.i_cost8x4[i],
                                         h->mb.i_sub_partition[i], D_L0_8x4 );

                            xavs_mb_analyse_inter_p4x8( h, &analysis, i );
                            COPY2_IF_LT( i_cost8x8, analysis.l0.i_cost4x8[i],
                                         h->mb.i_sub_partition[i], D_L0_4x8 );

                            i_cost += i_cost8x8 - analysis.l0.me8x8[i].cost;
                        }
                        xavs_mb_cache_mv_p8x8( h, &analysis, i );
                    }
            ...
            if( h->mb.b_chroma_me )
            {
                ...
                xavs_mb_analyse_intra( h, &analysis, i_cost - analysis.i_satd_i8x8chroma );
            ...
            i_satd_inter = i_cost;
            i_satd_intra = XAVS_MIN3( analysis.i_satd_i16x16,
                                      analysis.i_satd_i8x8,
                                      analysis.i_satd_i4x4 );

            if( analysis.i_mbrd )
            {
                xavs_mb_analyse_p_rd( h, &analysis, XAVS_MIN(i_satd_inter, i_satd_intra) );
                i_type = P_L0;
                i_partition = D_16x16;
                i_cost = analysis.l0.me16x16.cost;
                ...
                h->mb.i_partition = i_partition;
                if( i_cost < COST_MAX )
                    xavs_mb_analyse_transform_rd( h, &analysis, &i_satd_inter, &i_cost );
                xavs_intra_rd( h, &analysis, i_satd_inter * 5/4 );
            }
            ...
            if( analysis.i_mbrd >= 2)
            {
                ...
                else if( i_partition == D_16x8 )
                {
                    ...
                    xavs_me_refine_qpel_rd( h, &analysis.l0.me16x8[0], analysis.i_lambda2, 0, 0 );
                    xavs_me_refine_qpel_rd( h, &analysis.l0.me16x8[1], analysis.i_lambda2, 8, 0 );
                }
                else if( i_partition == D_8x16 )
                {
                    h->mb.i_sub_partition[0] = h->mb.i_sub_partition[1] =
                    ...
                    xavs_me_refine_qpel_rd( h, &analysis.l0.me8x16[0], analysis.i_lambda2, 0, 0 );
                    xavs_me_refine_qpel_rd( h, &analysis.l0.me8x16[1], analysis.i_lambda2, 4, 0 );
                }
                else if( i_partition == D_8x8 )
                {
                    int i8x8;
                    ...
                    for( i8x8 = 0; i8x8 < 4; i8x8++ )
                    {
                        ...
                        {
                           xavs_me_refine_qpel_rd( h, &analysis.l0.me8x4[i8x8][0], analysis.i_lambda2, i8x8*4+0, 0 );
                           xavs_me_refine_qpel_rd( h, &analysis.l0.me8x4[i8x8][1], analysis.i_lambda2, i8x8*4+2, 0 );
                        }
                        else if( h->mb.i_sub_partition[i8x8] == D_L0_4x8 )
                        {
                           xavs_me_refine_qpel_rd( h, &analysis.l0.me4x8[i8x8][0], analysis.i_lambda2, i8x8*4+0, 0 );
                           xavs_me_refine_qpel_rd( h, &analysis.l0.me4x8[i8x8][1], analysis.i_lambda2, i8x8*4+1, 0 );
                        }
                        else if( h->mb.i_sub_partition[i8x8] == D_L0_4x4 )
                        {
                           xavs_me_refine_qpel_rd( h, &analysis.l0.me4x4[i8x8][0], analysis.i_lambda2, i8x8*4+0, 0 );
                           xavs_me_refine_qpel_rd( h, &analysis.l0.me4x4[i8x8][1], analysis.i_lambda2, i8x8*4+1, 0 );
                           xavs_me_refine_qpel_rd( h, &analysis.l0.me4x4[i8x8][2], analysis.i_lambda2, i8x8*4+2, 0 );
                           xavs_me_refine_qpel_rd( h, &analysis.l0.me4x4[i8x8][3], analysis.i_lambda2, i8x8*4+3, 0 );
                        }
                    }
                }
            }
    ...
    else if( h->sh.i_type == SLICE_TYPE_B )
    {
        ...
        if( !b_skip )
        {
            ...
            {
                xavs_mb_analyse_b_rd( h, &analysis, i_cost );
            ...
            {
                i_satd_inter = i_cost;
                xavs_mb_analyse_b_rd( h, &analysis, i_satd_inter );
                i_type = B_SKIP;
                i_cost = i_bskip_cost;
                i_partition = D_16x16;
            ...
            xavs_mb_analyse_intra( h, &analysis, i_satd_inter );

            if( analysis.i_mbrd )
            {
                xavs_mb_analyse_transform_rd( h, &analysis, &i_satd_inter, &i_cost );
                xavs_intra_rd( h, &analysis, i_satd_inter * 17/16 );
            }
            ...
            if( analysis.i_mbrd >= 2 && IS_INTRA( i_type ) )
                xavs_intra_rd_refine( h, &analysis );
            if( h->mb.i_subpel_refine >= 5 )
                xavs_refine_bidir( h, &analysis );
            ...
            if( analysis.i_mbrd >= 2 && i_type > B_DIRECT && i_type < B_SKIP )
            {
                ...
                if( i_partition == D_16x16 )
                {
                    if( i_type == B_L0_L0 )
                        ...
                        xavs_me_refine_bidir_rd( h, &analysis.l0.me16x16, &analysis.l1.me16x16, i_biweight, 0, analysis.i_lambda2 );
                }
                else if( i_partition == D_16x8 )
                {
                    for( i = 0; i < 2; i++ )
                    {
                        ...
                        if( analysis.i_mb_partition16x8[i] == D_L0_8x8 )
                            ...
                            xavs_me_refine_bidir_rd( h, &analysis.l0.me16x8[i], &analysis.l1.me16x8[i], i_biweight, i*2, analysis.i_lambda2 );
                    }
                }
                else if( i_partition == D_8x16 )
                {
                    for( i = 0; i < 2; i++ )
                    {
                        ...
                        if( analysis.i_mb_partition8x16[i] == D_L0_8x8 )
                            ...
                            xavs_me_refine_bidir_rd( h, &analysis.l0.me8x16[i], &analysis.l1.me8x16[i], i_biweight, i, analysis.i_lambda2 );
                    }
                }
                else if( i_partition == D_8x8 )
                {
                    for( i = 0; i < 4; i++ )
                    {
                        if( h->mb.i_sub_partition[i] == D_L0_8x8 )
                            ...
                            xavs_me_refine_bidir_rd( h, &analysis.l0.me8x8[i], &analysis.l1.me8x8[i], i_biweight, i, analysis.i_lambda2 );
                    }
                }
            }
        }
...
static void xavs_analyse_update_cache( xavs_t *h, xavs_mb_analysis_t *a  )
{
    ...
    switch( h->mb.i_type )
    {
        case I_8x8:
            for( i = 0; i < 4; i++ )
                xavs_macroblock_cache_intra8x8_pred( h, 2*(i&1), 2*(i>>1), a->i_predict8x8[i] );
        ...
        case P_L0:
            switch( h->mb.i_partition )
            {
                ...
                case D_16x8:
                    ...
                    xavs_macroblock_cache_mv_ptr( h, 0, 0, 4, 2, 0, a->l0.me16x8[0].mv );
                    xavs_macroblock_cache_mv_ptr( h, 0, 2, 4, 2, 0, a->l0.me16x8[1].mv );
                    break;

                case D_8x16:
                    xavs_macroblock_cache_ref( h, 0, 0, 2, 4, 0, a->l0.me8x16[0].i_ref );
                    xavs_macroblock_cache_ref( h, 2, 0, 2, 4, 0, a->l0.me8x16[1].i_ref );
                    xavs_macroblock_cache_mv_ptr( h, 0, 0, 2, 4, 0, a->l0.me8x16[0].mv );
                    xavs_macroblock_cache_mv_ptr( h, 2, 0, 2, 4, 0, a->l0.me8x16[1].mv );
                    break;
        ...
        case P_SKIP:
        {
            ...
            xavs_macroblock_cache_mv_ptr( h, 0, 0, 4, 4, 0, h->mb.cache.pskip_mv );
            break;
        }
        ...
        default: /* the rest of the B types */
            ...
            case D_16x16:
                ...
                case B_L0_L0:
                    ...
                    xavs_macroblock_cache_mvd( h, 0, 0, 4, 4, 1, 0 );
                    break;
                case B_L1_L1:
                    xavs_macroblock_cache_ref( h, 0, 0, 4, 4, 0, -1 );
                    xavs_macroblock_cache_mv ( h, 0, 0, 4, 4, 0, 0 );
                    xavs_macroblock_cache_mvd( h, 0, 0, 4, 4, 0, 0 );

                    xavs_macroblock_cache_ref( h, 0, 0, 4, 4, 1, a->l1.i_ref );
                    xavs_macroblock_cache_mv_ptr( h, 0, 0, 4, 4, 1, a->l1.me16x16.mv );
                    break;
                case B_BI_BI:
                    xavs_macroblock_cache_ref( h, 0, 0, 4, 4, 0, a->l0.i_ref );
                    xavs_macroblock_cache_mv_ptr( h, 0, 0, 4, 4, 0, a->l0.me16x16.mv );
    ...
    if( h->param.i_threads > 1 && !IS_INTRA(h->mb.i_type) )
    {
        ...
        {
            int completed;
            int ref = h->mb.cache.ref[l][xavs_scan8[0]];
            if( ref < 0 )
                continue;
            completed = (l ? h->fref1 : h->fref0)[ ref >> h->mb.b_interlaced ]->i_lines_completed;
            if( (h->mb.cache.mv[l][xavs_scan8[15]][1] >> (2 - h->mb.b_interlaced)) + h->mb.i_mb_y*16 > completed )
            {
                xavs_log( h, XAVS_LOG_WARNING, "internal error (MV out of thread range)\n");
                fprintf(stderr, "mb type: %d \n", h->mb.i_type);
                fprintf(stderr, "mv: l%dr%d (%d,%d) \n", l, ref,
                                h->mb.cache.mv[l][xavs_scan8[15]][0],
                                h->mb.cache.mv[l][xavs_scan8[15]][1] );
                fprintf(stderr, "limit: %d \n", h->mb.mv_max_spel[1]);
                fprintf(stderr, "mb_xy: %d,%d \n", h->mb.i_mb_x, h->mb.i_mb_y);
                fprintf(stderr, "completed: %d \n", completed );
                xavs_log( h, XAVS_LOG_WARNING, "recovering by using intra mode\n");
                xavs_mb_analyse_intra( h, a, COST_MAX );
                h->mb.i_type = I_16x16;
                h->mb.i_intra16x16_pred_mode = a->i_predict16x16;
                xavs_mb_analyse_intra_chroma( h, a );
            }
