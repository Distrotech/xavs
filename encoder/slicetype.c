...
static int xavs_lowres_context_init( xavs_t *h, xavs_mb_analysis_t *a )
{
    a->i_qp = 12; // arbitrary, but low because SATD scores are 1/4 normal
    a->i_lambda = xavs_lambda_tab[ a->i_qp ];
    if( xavs_mb_analyse_load_costs( h, a ) )
        return -1;
...
int xavs_lowres_context_alloc( xavs_t *h )
{
    ...
    return xavs_lowres_context_init( h, &a );
}

static int xavs_slicetype_mb_cost( xavs_t *h, xavs_mb_analysis_t *a,
                            xavs_frame_t **frames, int p0, int p1, int b,
...
#define TRY_BIDIR( mv0, mv1, penalty ) \
    ...
    if( b_bidir )
    {
        ...
        h->mc.memcpy_aligned( &m[1], &m[0], sizeof(xavs_me_t) );
...
#undef MVC
            xavs_median_mv( m[l].mvp, mvc[0], mvc[1], mvc[2] );
...
lowres_intra_mb:
    ...
    if( !b_bidir || h->param.rc.b_mb_tree )
    {
        ...
        if( !fenc->b_intra_calculated )
        {
            ...
            i_icost = XAVS_MIN4( satds[0], satds[1], satds[2], satds[3] );
...
static int xavs_slicetype_frame_cost( xavs_t *h, xavs_mb_analysis_t *a,
                               xavs_frame_t **frames, int p0, int p1, int b,
                               int b_intra_penalty )
{
    ...
    else
    {
            ...
            h->sps->i_mb_width <= 2 || h->sps->i_mb_height <= 2 )
        {
            for( h->mb.i_mb_y = h->sps->i_mb_height - 1; h->mb.i_mb_y >= 0; h->mb.i_mb_y-- )
            {
                ...
                {
                    int i_mb_cost = xavs_slicetype_mb_cost( h, a, frames, p0, p1, b, dist_scale_factor, do_search );
...
static int xavs_slicetype_frame_cost_recalculate( xavs_t *h, xavs_mb_analysis_t *a, xavs_frame_t **frames,
                                                  int p0, int p1, int b )
{
    int i_score = 0;
    int *row_satd = frames[b]->i_row_satds[b-p0][p1-b];
    ...
    for( h->mb.i_mb_y = h->sps->i_mb_height - 1; h->mb.i_mb_y >= 0; h->mb.i_mb_y-- )
    {
        ...
        for( h->mb.i_mb_x = h->sps->i_mb_width - 1; h->mb.i_mb_x >= 0; h->mb.i_mb_x-- )
        {
            ...
            i_mb_cost = (i_mb_cost * xavs_exp2fix8(qp_adj*(-1.f/6.f)) + 128) >> 8;
...
static void xavs_macroblock_tree_propagate( xavs_t *h, xavs_frame_t **frames, int p0, int p1, int b )
{
    xavs_frame_t *refs[2] = {frames[p0],frames[p1]};
...
#define CLIP_ADD(s,x) (s) = XAVS_MIN((s)+(x),(1<<16)-1)
...
static void xavs_macroblock_tree( xavs_t *h, xavs_mb_analysis_t *a, xavs_frame_t **frames, int num_frames, int b_intra )
{
    int i, idx = !b_intra;
    int last_nonb, cur_nonb = 1;
    if( b_intra )
       xavs_slicetype_frame_cost( h, a, frames, 0, 0, 0, 0 );

    i = num_frames-1;
    while( i > 0 && frames[i]->i_type == XAVS_TYPE_B )
        i--;
    last_nonb = i;
    ...
    {
        cur_nonb = i;
        while( frames[cur_nonb]->i_type == XAVS_TYPE_B && cur_nonb > 0 )
            cur_nonb--;
        if( cur_nonb < idx )
            break;
        xavs_slicetype_frame_cost( h, a, frames, cur_nonb, last_nonb, last_nonb, 0 );
        memset( frames[cur_nonb]->i_propagate_cost, 0, h->mb.i_mb_count * sizeof(uint16_t) );
        xavs_macroblock_tree_propagate( h, frames, cur_nonb, last_nonb, last_nonb );
        while( frames[i]->i_type == XAVS_TYPE_B && i > 0 )
        {
            xavs_slicetype_frame_cost( h, a, frames, cur_nonb, last_nonb, i, 0 );
            memset( frames[i]->i_propagate_cost, 0, h->mb.i_mb_count * sizeof(uint16_t) );
            xavs_macroblock_tree_propagate( h, frames, cur_nonb, last_nonb, i );
            i--;
        }
        last_nonb = cur_nonb;
    }
    ...
    for( h->mb.i_mb_y = 0; h->mb.i_mb_y < h->sps->i_mb_height; h->mb.i_mb_y++ )
    {
        for( h->mb.i_mb_x = 0; h->mb.i_mb_x < h->sps->i_mb_width; h->mb.i_mb_x++ )
        {
            ...
            if( intra_cost )
            {
                ...
                float log2_ratio = xavs_log2(intra_cost + propagate_cost) - xavs_log2(intra_cost);
...
static int xavs_slicetype_path_cost( xavs_t *h, xavs_mb_analysis_t *a, xavs_frame_t **frames, char *path, int threshold )
{
    int loc = 1;
    int cost = 0;
    int cur_p = 0;
    ...
    while( path[loc] )
    {
        ...
        cost += xavs_slicetype_frame_cost( h, a, frames, cur_p, next_p, next_p, 0 );
        /* Early terminate if the cost we have found is larger than the best path cost so far */
        if( cost > threshold )
            break;

        for( next_b = loc; next_b < next_p && cost < threshold; next_b++ )
            cost += xavs_slicetype_frame_cost( h, a, frames, cur_p, next_p, next_b, 0 );
...
static void xavs_slicetype_path( xavs_t *h, xavs_mb_analysis_t *a, xavs_frame_t **frames, int length, int max_bframes, int buffer_size, char (*best_paths)[XAVS_LOOKAHEAD_MAX] )
{
    char paths[XAVS_BFRAME_MAX+2][XAVS_LOOKAHEAD_MAX] = {{0}};
    int num_paths = XAVS_MIN(max_bframes+1, length);
    int suffix_size, loc, path;
    int best_cost = COST_MAX;
    int best_path_index = 0;
    length = XAVS_MIN(length,XAVS_LOOKAHEAD_MAX);
    ...
    {
        int cost = xavs_slicetype_path_cost( h, a, frames, paths[path], best_cost );
        if( cost < best_cost )
        {
            best_cost = cost;
...
static int scenecut( xavs_t *h, xavs_mb_analysis_t *a, xavs_frame_t **frames, int p0, int p1 )
{
    xavs_frame_t *frame = frames[p1];
    ...
    xavs_slicetype_frame_cost( h, a, frames, p0, p1, p1, 0 );
    ...
    if( res )
    {
        ...
        int pmb = NUM_MBS - imb;
        xavs_log( h, XAVS_LOG_DEBUG, "scene cut at %d Icost:%d Pcost:%d ratio:%.4f bias:%.4f gop:%d (imb:%d pmb:%d)\n",
                  frame->i_frame,
...
static void xavs_slicetype_analyse( xavs_t *h, int keyframe )
{
    xavs_mb_analysis_t a;
    ...
    idr_frame_type = frames[1]->i_frame - h->frames.i_last_idr >= h->param.i_keyint_min ? XAVS_TYPE_IDR : XAVS_TYPE_I;
...
//FIXME: removed by zzf    if( h->param.i_scenecut_threshold && scenecut( h, &a, frames, 0, 1 ) )
    ...
    if( h->param.i_bframe )
    {
        if( h->param.i_bframe_adaptive == XAVS_B_ADAPT_TRELLIS )
        {
            ...
            for( n = 2; n < num_frames-1; n++ )
                xavs_slicetype_path( h, &a, frames, n, max_bframes, num_frames-max_bframes, best_paths );
            num_bframes = strspn( best_paths[num_frames-2], "B" );
            /* Load the results of the analysis into the frame types. */
            for( j = 1; j < num_frames; j++ )
                frames[j]->i_type = best_paths[num_frames-2][j-1] == 'B' ? XAVS_TYPE_B : XAVS_TYPE_P;
            frames[num_frames]->i_type = XAVS_TYPE_P;
        }
        else if( h->param.i_bframe_adaptive == XAVS_B_ADAPT_FAST )
        {
            for( i = 0; i < num_frames-(2-!i); )
            {
                cost2p1 = xavs_slicetype_frame_cost( h, &a, frames, i+0, i+2, i+2, 1 );
                if( frames[i+2]->i_intra_mbs[2] > i_mb_count / 2 )
                {
                    frames[i+1]->i_type = XAVS_TYPE_P;
                    frames[i+2]->i_type = XAVS_TYPE_P;
                    i += 2;
                    continue;
                }

                cost1b1 = xavs_slicetype_frame_cost( h, &a, frames, i+0, i+2, i+1, 0 );
                cost1p0 = xavs_slicetype_frame_cost( h, &a, frames, i+0, i+1, i+1, 0 );
                cost2p0 = xavs_slicetype_frame_cost( h, &a, frames, i+1, i+2, i+2, 0 );
                ...
                frames[i+1]->i_type = XAVS_TYPE_B;
                frames[i+2]->i_type = XAVS_TYPE_P;

                for( j = i+2; j <= XAVS_MIN( h->param.i_bframe, num_frames-2 ); j++ )
                {
                    int pthresh = XAVS_MAX(INTER_THRESH - P_SENS_BIAS * (j-i-1), INTER_THRESH/10);
                    int pcost = xavs_slicetype_frame_cost( h, &a, frames, i+0, j+1, j+1, 1 );
            ...
            frames[i+!i]->i_type = XAVS_TYPE_P;
            num_bframes = 0;
            while( num_bframes < num_frames && frames[num_bframes+1]->i_type == XAVS_TYPE_B )
                num_bframes++;
        }
        else
        {
            num_bframes = XAVS_MIN(num_frames-1, h->param.i_bframe);
            for( j = 1; j < num_frames; j++ )
                frames[j]->i_type = (j%(num_bframes+1)) ? XAVS_TYPE_B : XAVS_TYPE_P;
            frames[num_frames]->i_type = XAVS_TYPE_P;
        }
        ...
        reset_start = keyframe ? 1 : XAVS_MIN( num_bframes+2, num_analysed_frames+1 );
    }
    else
    {
        for( j = 1; j < num_frames; j++ )
    ...
    if( h->param.rc.b_mb_tree )
        xavs_macroblock_tree( h, &a, frames, XAVS_MIN(num_analysed_frames, h->param.rc.i_lookahead), keyframe );
    ...
    for( j = reset_start; j <= num_frames; j++ )
        frames[j]->i_type = XAVS_TYPE_AUTO;
}
    ...
    for( bframes = 0;; bframes++ )
    {
        ...
        if( frm->i_frame - h->frames.i_last_idr >= h->param.i_keyint_max )
        {
            ...
            if( frm->i_type != XAVS_TYPE_IDR )
                xavs_log( h, XAVS_LOG_WARNING, "specified frame type (%d) is not compatible with keyframe interval\n", frm->i_type );
        }
        if( frm->i_type == XAVS_TYPE_IDR )
        {
            /* Close GOP */
            ...
            || h->frames.next[bframes+1] == NULL )
        {
            if( IS_XAVS_TYPE_B( frm->i_type ) )
                xavs_log( h, XAVS_LOG_WARNING, "specified frame type is not compatible with max B-frames\n" );
            if( frm->i_type == XAVS_TYPE_AUTO
                || IS_XAVS_TYPE_B( frm->i_type ) )
        ...
        else if( !IS_XAVS_TYPE_B( frm->i_type ) ) break;
    }
}
...
{
    xavs_mb_analysis_t a;
    xavs_frame_t *frames[XAVS_LOOKAHEAD_MAX+2] = { NULL, };
    int p0=0, p1, b;
    int cost;
    ...
    if( h->param.rc.b_mb_tree )
        cost = xavs_slicetype_frame_cost_recalculate( h, &a, frames, p0, p1, b );
    else
    {
        cost = xavs_slicetype_frame_cost( h, &a, frames, p0, p1, b, 0 );
