...
void xavs_sequence_end_write( bs_t *s )
{
   bs_write( s, 8, 0xB1 );
}
...
void xavs_sps_init( xavs_sps_t *sps, int i_id, xavs_param_t *param )
{
    ...
    sps->i_num_ref_frames = XAVS_MIN(16, param->i_frame_reference + sps->vui.i_num_reorder_frames + param->b_bframe_pyramid);
...
void xavs_sei_version_write( xavs_t *h, bs_t *s )
{
    ...
    sprintf( version, "xavs -  %d%s - AVS video codec - "
             "Copyleft 2009 - http://xavs.sourceforge.net - options: %s",
