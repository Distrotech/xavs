...
static void scaling_list_write( bs_t *s, xavs_pps_t *pps, int idx )
{
    const int len = idx<4 ? 16 : 64;
    const uint8_t *zigzag = idx<4 ? xavs_zigzag_scan4[0] : xavs_zigzag_scan8[0];
...
void xavs_sps_init( xavs_sps_t *sps, int i_id, xavs_param_t *param )
{
    ...
    /* xavs doesn't support the features that are in Baseline and not in Main,
    ...
    sps->i_num_ref_frames = XAVS_MIN(16, XAVS_MAX(param->i_frame_reference, 1 + sps->vui.i_num_reorder_frames));
...
void xavs_pps_init( xavs_pps_t *pps, int i_id, xavs_param_t *param, xavs_sps_t *sps )
{
    ...
    pps->i_pic_init_qp = param->rc.i_rc_method == XAVS_RC_ABR ? 26 : param->rc.i_qp_constant;
...
int xavs_sei_version_write( xavs_t *h, bs_t *s )
{
    int i;
...
{\
    if( verbose )\
        xavs_log( h, XAVS_LOG_WARNING, __VA_ARGS__ );\
    ret = 1;\
}

int xavs_validate_levels( xavs_t *h, int verbose )
{
    int ret = 0;
