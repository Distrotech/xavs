...
static int  Parse( int argc, char **argv,
                   xavs_param_t *param, cli_opt_t *opt )
{
	...
	for( optind = 0;; )
	{
        ...
        switch( c )
        {
            ...
            case 'A':
                ...
                if( strstr( optarg, "i4x4" ) )  param->analyse.inter |= XAVS_ANALYSE_I4x4;
                if( strstr( optarg, "i8x8" ) )  param->analyse.inter |= XAVS_ANALYSE_I8x8;
                if( strstr( optarg, "p8x8" ) )  param->analyse.inter |= XAVS_ANALYSE_PSUB16x16;
                if( strstr( optarg, "p4x4" ) )  param->analyse.inter |= XAVS_ANALYSE_PSUB8x8;
                if( strstr( optarg, "b8x8" ) )  param->analyse.inter |= XAVS_ANALYSE_BSUB16x16;
                break;
            case OPT_DIRECT:
...
generic_option:
    ...
    if( b_thread_input || param->i_threads > 1
        || (param->i_threads == XAVS_THREADS_AUTO && xavs_cpu_num_processors() > 1) )
    {
    ...
    if( !b_user_ref )
    {
        ...
        int i;
        for( i = 0; xavs_levels[i].level_idc != 0; i++ )
            if( param->i_level_idc == xavs_levels[i].level_idc )
            {
                while( mbs * 384 * param->i_frame_reference > xavs_levels[i].dpb
                       && param->i_frame_reference > 1 )
                {
...
static void parse_qpfile( cli_opt_t *opt, xavs_picture_t *pic, int i_frame )
{
    int num = -1, qp, ret;
    char type;
    uint64_t file_pos;
    while( num < i_frame )
    {
        ...
        if( num < i_frame && ret == 3 )
            continue;
        pic->i_qpplus1 = qp+1;
        if     ( type == 'I' ) pic->i_type = XAVS_TYPE_IDR;
        else if( type == 'i' ) pic->i_type = XAVS_TYPE_I;
        else if( type == 'P' ) pic->i_type = XAVS_TYPE_P;
        else if( type == 'B' ) pic->i_type = XAVS_TYPE_BREF;
        else if( type == 'b' ) pic->i_type = XAVS_TYPE_B;
        else ret = 0;
        if( ret != 3 || qp < -1 || qp > 51 )
        {
...
static int  Encode_frame( xavs_t *h, hnd_t hout, xavs_picture_t *pic )
{
...
static void Print_status( int64_t i_start, int i_frame, int i_frame_total, int64_t i_file, xavs_param_t *param )
{
    char    buf[200];
    ...
    if( i_frame_total )
    {
        ...
        sprintf( buf, "xavs [%.1f%%] %d/%d frames, %.2f fps, %.2f kb/s, eta %d:%02d:%02d",
