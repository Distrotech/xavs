/*****************************************************************************
 * xavs:  xavs encoder testing program.
 *****************************************************************************
...
static void Help( xavs_param_t *defaults, int b_longhelp );
...
#define H1 if(b_longhelp) printf
    ...
    H0( "Presets:\n" );
    H0( "\n" );
    H0( "      --profile               Force avs profile [high]\n" );
                   ...
                   xavs_param_t *param, cli_opt_t *opt )
{
    ...
    for( optind = 0;; )
    {
        ...
        if( c == OPT_PRESET )
        {
            ...
            {
                param->analyse.inter = XAVS_ANALYSE_I8x8|XAVS_ANALYSE_I4x4;
                param->analyse.i_me_method = XAVS_ME_DIA;
...
generic_option:
    ...
    if( profile )
    {
        if( !strcasecmp( profile, "baseline" ) )
        {
            ...
            {
                fprintf( stderr, "xavs [error]: baseline profile doesn't support interlacing\n" );
                return -1;
            }
        }
        ...
        {
            fprintf( stderr, "xavs [error]: invalid profile: %s\n", profile );
            return -1;
        }
        if( (param->rc.i_rc_method == XAVS_RC_CQP && param->rc.i_qp_constant == 0) ||
            (param->rc.i_rc_method == XAVS_RC_CRF && param->rc.f_rf_constant == 0) )
        {
            fprintf( stderr, "xavs [error]: %s profile doesn't support lossless\n", profile );
            return -1;
        }
    }
    ...
    if( b_thread_input || param->i_threads > 1
        || (param->i_threads == 0 && xavs_cpu_num_processors() > 1) )
    {
   ...
   fflush(hout);
    for( i = 0; i < i_nal; i++ )
    {
        ...
        if( mux_buffer_size < nal[i].i_payload * 3/2 + 4 )
        {
            ...
            xavs_free( mux_buffer );
            mux_buffer = xavs_malloc( mux_buffer_size );
            if( !mux_buffer )
                return -1;
        }

        i_size = mux_buffer_size;
        //xavs_nal_encode( mux_buffer, &i_size, 1, &nal[i] );
