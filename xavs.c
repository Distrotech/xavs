...
static void Help( xavs_param_t *defaults );
            ...
            for( psz = psz_filename; *psz; psz++ )
            {
                    ...
                    && sscanf( psz, "%ux%u", &param->i_width, &param->i_height ) == 2 )
                {
                    if( param->i_log_level >= XAVS_LOG_INFO )
                        fprintf( stderr, "xavs [info]: file name gives %dx%d\n", param->i_width, param->i_height );
                    break;
                }
            }
        }
...
static int  Encode( xavs_param_t *param, cli_opt_t *opt )
{
    ...
    {
        fprintf( stderr, "xavs_encoder_open failed\n" );
    ...
    /* Create a new pic */
    xavs_picture_alloc( &pic, XAVS_CSP_I420, param->i_width, param->i_height );
         ...
         b_ctrl_c == 0 && (i_frame < i_frame_total || i_frame_total == 0); )
    {
        ...
        if( opt->b_progress && param->i_log_level < XAVS_LOG_DEBUG && 
